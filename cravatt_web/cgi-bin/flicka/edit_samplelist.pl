...
# a simple editor for samplelist.txt that simply uses a <TEXTAREA> to hold and
# edit the file
...
if (!defined $input) {
  &MS_pages_header ("Sample List Editor", "#9F9F5F");
  &output_form;   # subroutine
  exit;
}
...
#qx{ $command };
# let's use Perl commands instead:
...
<tt><TEXTAREA ROWS=28 COLS=100 NAME="input" WRAP=off>$input</TEXTAREA></tt>
