...
if len(argv) != 4:
    print 'Usage: %s <peptide_sequence> <mono|mz|avg> <mod_mass>'%argv[0]
    print 'calculate modified peptide mass based on SEQUEST'
...
for pepseq in sequences:
    ...
    mass = mass + float(argv[3])
    print "mass: ", mass
    for ch in charges:
        print "charge ", ch, ": ", (mass+int(ch)*MonoHplus)/int(ch)
